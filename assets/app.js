...
(function () {
  ...
  /* ── 경기 카드 ──────────────────────────────────────────
     JS 클릭 핸들러를 없앴다. 카드는 이제 「`<article>` + 카드를 덮는 `<a class="card-link">`」
     구조이므로 브라우저가 알아서 이동한다 (index.html · assets/app.css).
     예전에는 `<article>` 에 role="link" + tabindex + data-href 를 달고 여기서 이동시켰는데,
     ① Lighthouse 가 「부적절한 ARIA 역할」로 지적했고
     ② 카드 안 팀명 링크와 겹쳐 「링크 안의 링크」가 됐다.
     ⚠ 카드 안에 버튼을 다시 넣게 되면 덮개(z-index:1)보다 위로 올려야 눌린다. */
